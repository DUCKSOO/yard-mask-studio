...
> **도구 사용법(화면 조작·단축키·작업 순서)**은 [user_guide.md](./user_guide.md) 를 참고한다. 본 문서는 **클래스 정의·경계 판정 기준** 에 집중한다.
...
3. **SAM (선택)** — 점(+)/점(−)/박스로 프롬프트를 두고 **SAM 실행**(또는 SAM 자동)으로 후보를 받는다. **여러 덩어리 자재는 양성을 덩어리마다 한 점씩** 찍는 것을 권장한다. 같은 자재는 **양성 1점 + 음성 점**으로 다듬는다. 후보가 없거나 부족하면 브러시로 직접 칠한다.
